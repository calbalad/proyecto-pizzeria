...
import com.pizzaiolo.authorization.models.dtos.UpdateUserPermissionDto;
import com.pizzaiolo.authorization.models.entities.User;
import com.pizzaiolo.authorization.services.FileStorageServiceImpl;
...
public class UserController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = SWG_USER_LOGGED_MESSAGE, response = UserResponse.class),
			@ApiResponse(code = 401, message = UNAUTHORIZED_MESSAGE, response = BadRequestResponse.class), })
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = SWG_USER_ITEM_MESSAGE, response = UserResponse.class),
			@ApiResponse(code = 401, message = UNAUTHORIZED_MESSAGE, response = BadRequestResponse.class),
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = SWG_USER_UPDATE_MESSAGE, response = UserResponse.class),
			@ApiResponse(code = 401, message = UNAUTHORIZED_MESSAGE, response = BadRequestResponse.class),
			@ApiResponse(code = 403, message = FORBIDDEN_MESSAGE, response = BadRequestResponse.class),
	...
	@ApiResponses(value = {
			...
			@ApiResponse(code = 403, message = FORBIDDEN_MESSAGE, response = BadRequestResponse.class),
			@ApiResponse(code = 422, message = INVALID_DATA_MESSAGE, response = InvalidDataResponse.class), })
	@PreAuthorize("hasAuthority('change:role')")
	@PutMapping("/{id}/role")
	public ResponseEntity<UserResponse> updateRol(@PathVariable String id, @Valid @RequestBody Role setRole)
			throws PasswordNotMatchException, ResourceNotFoundException {
		User user = userService.findById(id);
		user.setRole(roleService.findByName(setRole.getName()));
	...
	public ResponseEntity<UserResponse> updatePassword(@PathVariable String id,
	...
	public ResponseEntity<UserResponse> uploadPicture(@PathVariable String id,
	...
	public ResponseEntity<UserResponse> assignPermissions(@PathVariable String id,
			@Valid @RequestBody UpdateUserPermissionDto updateUserPermissionDto) throws ResourceNotFoundException {
		User user = userService.findById(id);
	...
	public ResponseEntity<User> revokePermissions(@PathVariable String id,
			@Valid @RequestBody UpdateUserPermissionDto updateUserPermissionDto) throws ResourceNotFoundException {
		User user = userService.findById(id);
